...
function generateStageRiskAnalysis(candidate: CandidateInput, profile: PositionFailureProfile) {
  // 简单模拟：假设候选人来自大厂
  const candidateOptimalStage: CompanyStage = 'enterprise';

  // 从 profile 获取当前公司阶段（需要从 position 获取，这里简化处理）
  const currentCompanyStage: CompanyStage = 'startup';

  // 计算匹配分数（这里故意设置为不同阶段，模拟不匹配场景）
  // @ts-expect-error - Mock data intentionally uses different stages for demonstration
